...
> **Entschieden (ADR-011, 2026-07-26):** Diese App-API ist ausschließlich bewohnerbezogen (`PowerParticipant`, `assertParticipantScope`) — und bleibt es. Die **Kunden-App ist eine Bewohner-App**; Vermieter und Hausverwaltung arbeiten über die Web-Plattform (`apps/platform`). Ein zweiter, objekt-scoped Endpunktsatz wird nicht gebaut. Begründung, Konsequenzen und der Revisionsweg stehen in `docs/DECISIONS/ADR-011-kunden-app-nur-bewohner.md`.
